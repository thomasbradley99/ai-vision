#!/usr/bin/env python3
"""
Track ALL people in video, then match yourself to one of the tracks.

This is better than track_specific_person.py because:
1. First: Detects and tracks EVERYONE in the video
2. Then: Matches your reference image to one of the tracks
3. Result: You can see all people, and you're highlighted

Usage:
    python track_all_people_then_match.py \
        --video inputs/jj25_nov.mov \
        --reference-image inputs/reference.jpg \
        --output outputs/all_people_tracked.mp4 \
...
def find_matching_track(reference_embedding: torch.Tensor, track_summaries: List[Dict], 
                        similarity_threshold: float = 0.7) -> Optional[int]:
    """Find which track matches the reference person."""
    ...
    for track_summary in track_summaries:
        ...
        track_embedding = torch.tensor(track_summary["embedding"])
        ...
        print(f"  Track {track_id}: similarity = {similarity:.3f}")
    
    if best_similarity >= similarity_threshold:
        print(f"\n[Matching] ✓ Found match! Track {best_match_id} (similarity: {best_similarity:.3f})")
        return best_match_id
    else:
        print(f"\n[Matching] ✗ No match found above threshold {similarity_threshold}")
        print(f"  Best match: Track {best_match_id} (similarity: {best_similarity:.3f})")
        print(f"  Try lowering --similarity-threshold or use a better reference image")
        return None
...
def main():
    parser = argparse.ArgumentParser(
        description="Track all people, then match yourself to one track"
    )
    parser.add_argument(
        "--video",
        type=Path,
    ...
    print("=" * 60)
    print("Track All People, Then Match Yourself")
    print("=" * 60)
    ...
    # Step 1: Detect and track ALL people
    ...
    # Step 2: Compute embeddings for all tracks
    ...
    print(f"[Step 2] ✓ Computed embeddings for {len(track_summaries)} tracks")
